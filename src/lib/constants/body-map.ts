// Polygon shape data adapted from react-body-highlighter (MIT License)
// https://github.com/giavinh79/react-body-highlighter
// Coordinates kept as-is; grouped into fitness-oriented regions below.
// Ported verbatim from the web app (lib/constants/bodyMap.ts) so the RN
// anatomy selector renders the exact same silhouette as web.
...
export const ANTERIOR_POLYGONS: PolygonShape[] = [
  {
    key: 'chest',
    points: [
...
export const POSTERIOR_POLYGONS: PolygonShape[] = [
  {
    key: 'head',
    points: [
...
// Body parts drawn for a complete silhouette but not selectable.
export const DECORATIVE_KEYS = ['head', 'neck', 'knees'];

export type BodyRegion = {
  id: string;
  labelKey: string;
  anteriorKeys: string[];
  posteriorKeys: string[];
  muscleValues: string[];
};
...
export const BODY_REGIONS: BodyRegion[] = [
  ...
  {
    id: 'shoulders_front',
    labelKey: 'region_shoulders_front',
    anteriorKeys: ['front-deltoids'],
    posteriorKeys: [],
    muscleValues: ['deltoid_anterior', 'deltoid_lateral'],
  },
  {
    id: 'shoulders_rear',
    labelKey: 'region_shoulders_rear',
    anteriorKeys: [],
    posteriorKeys: ['back-deltoids'],
    muscleValues: [
      'deltoid_posterior',
      'supraspinatus',
      'infraspinatus',
  ...
  {
    id: 'triceps',
    labelKey: 'region_triceps',
    anteriorKeys: ['triceps'],
    posteriorKeys: ['triceps'],
    muscleValues: [
  ...
  {
    id: 'forearm',
    labelKey: 'region_forearm',
    anteriorKeys: ['forearm'],
    posteriorKeys: ['forearm'],
    muscleValues: [
      'brachioradialis',
      'pronator_teres',
      'pronator_quadratus',
  ...
  },
  {
    ...
    anteriorKeys: ['abs'],
    posteriorKeys: [],
    muscleValues: ['rectus_abdominis', 'transverse_abdominis', 'diaphragm'],
  },
  {
    id: 'obliques',
    labelKey: 'region_obliques',
    anteriorKeys: ['obliques'],
    posteriorKeys: [],
    muscleValues: ['external_oblique', 'internal_oblique'],
  },
  {
    id: 'trapezius',
    labelKey: 'region_trapezius',
    anteriorKeys: [],
    posteriorKeys: ['trapezius'],
    muscleValues: [
      'trapezius_superior',
      'trapezius_middle',
      'trapezius_inferior',
  ...
  {
    id: 'lats_upper_back',
    labelKey: 'region_lats_upper_back',
    anteriorKeys: [],
    posteriorKeys: ['upper-back'],
    muscleValues: [
      'latissimus_dorsi',
      'rhomboid_major',
      'rhomboid_minor',
  ...
  {
    id: 'lower_back',
    labelKey: 'region_lower_back',
    anteriorKeys: [],
    posteriorKeys: ['lower-back'],
    muscleValues: ['erector_spinae', 'quadratus_lumborum', 'multifidus'],
  },
  {
    id: 'quadriceps',
    labelKey: 'region_quadriceps',
    anteriorKeys: ['quadriceps'],
    posteriorKeys: [],
    muscleValues: [
      'rectus_femoris',
      'vastus_lateralis',
  ...
  {
    id: 'hamstring',
    labelKey: 'region_hamstring',
    anteriorKeys: [],
    posteriorKeys: ['hamstring'],
    muscleValues: [
  ...
  {
    id: 'gluteal',
    labelKey: 'region_gluteal',
    anteriorKeys: [],
    posteriorKeys: ['gluteal'],
    muscleValues: [
      'gluteus_maximus',
      'gluteus_medius',
      'gluteus_minimus',
  ...
  {
    id: 'hip_inner',
    labelKey: 'region_hip_inner',
    anteriorKeys: ['abductors'],
    posteriorKeys: ['adductor'],
    muscleValues: [
      'iliopsoas',
  ...
  {
    id: 'calves',
    labelKey: 'region_calves',
    anteriorKeys: ['calves'],
    posteriorKeys: ['calves', 'left-soleus', 'right-soleus'],
    muscleValues: [
...
const buildPolygonToRegion = (view: BodyView): Record<string, string> => {
  const map: Record<string, string> = {};
  BODY_REGIONS.forEach((region) => {
    const keys = view === 'anterior' ? region.anteriorKeys : region.posteriorKeys;
    keys.forEach((key) => {
      map[key] = region.id;
    });
  });
...
export const ANTERIOR_POLYGON_TO_REGION = buildPolygonToRegion('anterior');
export const POSTERIOR_POLYGON_TO_REGION = buildPolygonToRegion('posterior');

// Legacy / aggregate tokens stored in older records that have no fine-grained
// equivalent. Mapped to a region so existing data still highlights the body map.
export const LEGACY_VALUE_TO_REGION: Record<string, string> = {
  select_all_chest: 'chest',
  shoulders: 'shoulders_front',
  select_all_deltoid: 'shoulders_front',
  rotator_cuff: 'shoulders_rear',
  biceps_group: 'biceps',
  triceps_group: 'triceps',
  forearm_group: 'forearm',
  core: 'abs',
  core_group: 'abs',
  select_all_core: 'abs',
  transversus_abdominis: 'abs',
  trapezius: 'trapezius',
  upper_back: 'lats_upper_back',
  lower_back: 'lower_back',
  quadriceps: 'quadriceps',
  hamstrings_group: 'hamstring',
  glutes_group: 'gluteal',
  adductors_group: 'hip_inner',
  calves_group: 'calves',
  gastrocnemius: 'calves',
};

export const MUSCLE_VALUE_TO_REGION: Record<string, string> = BODY_REGIONS.reduce<
  Record<string, string>
>(
  (acc, region) => {
    region.muscleValues.forEach((value) => {
      acc[value] = region.id;
    });
    return acc;
  },
  { ...LEGACY_VALUE_TO_REGION },
);

export const BODY_REGION_BY_ID: Record<string, BodyRegion> = BODY_REGIONS.reduce<
  Record<string, BodyRegion>
>((acc, region) => {
  acc[region.id] = region;
  return acc;
}, {});

export const SVG_VIEW_BOX = { minX: -2, minY: -2, width: 104, height: 226 };
